...
public class RegisterVehicleCommandValidator : AbstractValidator<RegisterVehicleCommand>
{
    ...
    public RegisterVehicleCommandValidator()
    {
        RuleFor(c => c.LicensePlate)
            ...
            .MinimumLength(MinimumLicensePlateLength)
                .WithMessage($"A placa do veículo deve ter pelo menos {MinimumLicensePlateLength} caracteres.")
            .MaximumLength(MaximumLicensePlateLength)
                .WithMessage($"A placa do veículo deve ter no máximo {MaximumLicensePlateLength} caracteres.");
        ...
        RuleFor(c => c.PhotoBytes)
            ...
            .Must(bytes => bytes.Length > 0)
                .WithMessage("A foto do veículo não pode estar vazia.");
    }
}
